...
max_err_pos { self . expected . insert ( expected ) ; } Failed } } struct ParseState < 'input > { max_err_pos : usize , expected : :: std :: collections :: HashSet < & 'static str > , _phantom : :: std :: marker :: PhantomData < & 'input ( ) > , } impl < 'input > ParseState < 'input > { fn new ( ) -> ParseState < 'input > { ParseState { max_err_pos : 0 , expected : :: std :: collections :: HashSet :: new ( ) , _phantom : :: std :: marker :: PhantomData , } } } 
 ...
 fn parse_stuff < 'input > ( __input : & 'input str , __state : & mut ParseState < 'input > , __pos : usize ) -> RuleResult < Vec<Pos> > { # ! [ allow ( non_snake_case , unused ) ] { let seq_res = { let mut __repeat_pos = __pos ; let mut repeat_value = vec ! ( ) ; loop { let __pos = __repeat_pos ; let step_res = parse_delimited_stuff ( __input , __state , __pos ) ; match step_res { Matched ( newpos , value ) => { __repeat_pos = newpos ; repeat_value . push ( value ) ; } , Failed => { break ; } } } Matched ( __repeat_pos , repeat_value ) } ; match seq_res { Matched ( __pos , stuffs ) => { Matched ( __pos , {  stuffs  } ) } Failed => Failed , } } } 

 fn parse_delimited_stuff < 'input > ( __input : & 'input str , __state : & mut ParseState < 'input > , __pos : usize ) -> RuleResult < Pos > { # ! [ allow ( non_snake_case , unused ) ] { let seq_res = Matched ( __pos , __pos ) ; match seq_res { Matched ( __pos , start ) => { { let seq_res = { let choice_res = parse_plain_stuff ( __input , __state , __pos ) ; match choice_res { Matched ( __pos , __value ) => Matched ( __pos , __value ) , Failed => { let choice_res = parse_quoted_stuff ( __input , __state , __pos ) ; match choice_res { Matched ( __pos , __value ) => Matched ( __pos , __value ) , Failed => { let choice_res = parse_comment_stuff ( __input , __state , __pos ) ; match choice_res { Matched ( __pos , __value ) => Matched ( __pos , __value ) , Failed => parse_variable_stuff ( __input , __state , __pos ) } } } } } } ; match seq_res { Matched ( __pos , stuff ) => { { let seq_res = Matched ( __pos , __pos ) ; match seq_res { Matched ( __pos , end ) => { Matched ( __pos , {  Pos { start: start, end: end, token: stuff }  } ) } Failed => Failed , } } } Failed => Failed , } } } Failed => Failed , } } } 

 fn parse_plain_stuff < 'input > ( __input : & 'input str , __state : & mut ParseState < 'input > , __pos : usize ) -> RuleResult < Token > { # ! [ allow ( non_snake_case , unused ) ] { let seq_res = parse_plain ( __input , __state , __pos ) ; match seq_res { Matched ( __pos , _ ) => { Matched ( __pos , {  Text  } ) } Failed => Failed , } } } 

 fn parse_quoted_stuff < 'input > ( __input : & 'input str , __state : & mut ParseState < 'input > , __pos : usize ) -> RuleResult < Token > { # ! [ allow ( non_snake_case , unused ) ] { let seq_res = { let choice_res = parse_single_quoted ( __input , __state , __pos ) ; match choice_res { Matched ( __pos , __value ) => Matched ( __pos , __value ) , Failed => parse_double_quoted ( __input , __state , __pos ) } } ; match seq_res { Matched ( __pos , _ ) => { Matched ( __pos , {  Text  } ) } Failed => Failed , } } } 

 fn parse_comment_stuff < 'input > ( __input : & 'input str , __state : & mut ParseState < 'input > , __pos : usize ) -> RuleResult < Token > { # ! [ allow ( non_snake_case , unused ) ] { let seq_res = parse_line_comment ( __input , __state , __pos ) ; match seq_res { Matched ( __pos , _ ) => { Matched ( __pos , {  Text  } ) } Failed => Failed , } } } 

 fn parse_variable_stuff < 'input > ( __input : & 'input str , __state : & mut ParseState < 'input > , __pos : usize ) -> RuleResult < Token > { # ! [ allow ( non_snake_case , unused ) ] { let seq_res = slice_eq ( __input , __state , __pos , "{" ) ; match seq_res { Matched ( __pos , _ ) => { { let seq_res = { let mut __repeat_pos = __pos ; loop { let __pos = __repeat_pos ; let step_res = parse_whitespace ( __input , __state , __pos ) ; match step_res { Matched ( newpos , value ) => { __repeat_pos = newpos ; } , Failed => { break ; } } } Matched ( __repeat_pos , ( ) ) } ; match seq_res { Matched ( __pos , _ ) => { { let seq_res = match slice_eq ( __input , __state , __pos , "&" ) { Matched ( newpos , value ) => { Matched ( newpos , Some ( value ) ) } , Failed => { Matched ( __pos , None ) } , } ; match seq_res { Matched ( __pos , ref ) => { { let seq_res = { let mut __repeat_pos = __pos ; loop { let __pos = __repeat_pos ; let step_res = parse_whitespace ( __input , __state , __pos ) ; match step_res { Matched ( newpos , value ) => { __repeat_pos = newpos ; } , Failed => { break ; } } } Matched ( __repeat_pos , ( ) ) } ; match seq_res { Matched ( __pos , _ ) => { { let seq_res = parse_name ( __input , __state , __pos ) ; match seq_res { Matched ( __pos , var ) => { { let seq_res = { let mut __repeat_pos = __pos ; loop { let __pos = __repeat_pos ; let step_res = parse_whitespace ( __input , __state , __pos ) ; match step_res { Matched ( newpos , value ) => { __repeat_pos = newpos ; } , Failed => { break ; } } } Matched ( __repeat_pos , ( ) ) } ; match seq_res { Matched ( __pos , _ ) => { { let seq_res = slice_eq ( __input , __state , __pos , "}" ) ; match seq_res { Matched ( __pos , _ ) => { Matched ( __pos , { 
          let mode = if ref.is_some() { Identifier } else { Parameter }
 ...
 fn parse_whitespace < 'input > ( __input : & 'input str , __state : & mut ParseState < 'input > , __pos : usize ) -> RuleResult < () > { # ! [ allow ( non_snake_case , unused ) ] if __input . len ( ) > __pos { let ( __ch , __next ) = char_range_at ( __input , __pos ) ; match __ch { ' ' | '\t' | '\n' | '\r' => Matched ( __next , ( ) ) , _ => __state . mark_failure ( __pos , "[ \t\n\r]" ) , } } else { __state . mark_failure ( __pos , "[ \t\n\r]" ) } } pub fn stuff < 'input > ( input : & 'input str ) -> ParseResult < Vec<Pos> > { # ! [ allow ( non_snake_case , unused ) ] let mut state = ParseState :: new ( ) ; match parse_stuff ( input , & mut state , 0 ) { Matched ( pos , value ) => { if pos == input . len ( ) { return Ok ( value ) } } _ => { } } let ( line , col ) = pos_to_line ( input , state . max_err_pos ) ; Err ( ParseError { line : line , column : col , offset : state . max_err_pos , expected : state . expected , } ) } pub fn variable_stuff < 'input > ( input : & 'input str ) -> ParseResult < Token > { # ! [ allow ( non_snake_case , unused ) ] let mut state = ParseState :: new ( ) ; match parse_variable_stuff ( input , & mut state , 0 ) { Matched ( pos , value ) => { if pos == input . len ( ) { return Ok ( value ) } } _ => { } } let ( line , col ) = pos_to_line ( input , state . max_err_pos ) ; Err ( ParseError { line : line , column : col , offset : state . max_err_pos , expected : state . expected , } ) }